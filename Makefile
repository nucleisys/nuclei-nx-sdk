...
ifeq ($(core_name),nx605) 
RISCV_ARCH := rv64imac
RISCV_ABI  := lp64
endif

ifeq ($(core_name),nx607) 
...
RISCV_COMPILER ?= ./prebuilt_tools/riscv-nuclei-elf-gcc
#RISCV_COMPILER ?= ./prebuilt_tools/riscv64-unknown-elf-gcc

RISCV_GCC     := $(abspath $(RISCV_COMPILER)/bin/riscv-nuclei-elf-gcc)
RISCV_GXX     := $(abspath $(RISCV_COMPILER)/bin/riscv-nuclei-elf-g++)
RISCV_OBJDUMP := $(abspath $(RISCV_COMPILER)/bin/riscv-nuclei-elf-objdump)
RISCV_OBJCOPY := $(abspath $(RISCV_COMPILER)/bin/riscv-nuclei-elf-objcopy)
RISCV_GDB     := $(abspath $(RISCV_COMPILER)/bin/riscv-nuclei-elf-gdb)
RISCV_AR      := $(abspath $(RISCV_COMPILER)/bin/riscv-nuclei-elf-ar)
RISCV_SIZE    := $(abspath $(RISCV_COMPILER)/bin/riscv-nuclei-elf-size)
